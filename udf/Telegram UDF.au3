...
;~ 	  $msgData = _JSONDecodeTLGM($newUpdates)
;~ 	  $offset = $msgData[0] + 1
;~ 	  Return $msgData
;~    WEnd
;~ EndFunc
...
#ce ===============================================================================
Func _SendMsg($ChatID=$DEVChatID, $Text="test", $ParseMode = Default, $KeyboardMarkup = Default, $ResizeKeyboard = False, $OneTimeKeyboard = False, $DisableWebPreview = False, $DisableNotification = true)
   Local $Query = "https://api.telegram.org/" & $token & "/sendMessage?chat_id=" & $ChatID &"&text=" & $Text
...
Func _ForwardMsg($ChatID, $OriginalChatID, $MsgID, $DisableNotification = False)
   Local $Query = "https://api.telegram.org/" & $token & "/forwardMessage?chat_id=" & $ChatID & "&from_chat_id=" & $OriginalChatID & "&message_id=" & $MsgID
   If $DisableNotification = True Then $Query &= "&disable_notification=True"
   Local $Response = HttpPost($Query)
   ;Return $Response
   Return True
...
Func _SendPhoto($ChatID, $Path, $Caption = "")
   Local $Query = "https://api.telegram.org/" & $token & '/sendPhoto'
...
Func _SendAudio($ChatID, $Path, $Caption = "")
   Local $Query = "https://api.telegram.org/" & $token & '/sendAudio'
   Local $hOpen = _WinHttpOpen()
   Local $Form = '<form action="' & $Query & '" method="post" enctype="multipart/form-data">' & _
				  ' <input type="text" name="chat_id"/>' & _
...
Func _SendVideo($ChatID, $Path, $Caption = "")
   Local $Query = "https://api.telegram.org/" & $token & '/sendVideo'
   Local $hOpen = _WinHttpOpen()
   Local $Form = '<form action="' & $Query & '" method="post" enctype="multipart/form-data">' & _
				  ' <input type="text" name="chat_id"/>' & _
...
Func _SendDocument($ChatID, $Path, $Caption = "")
   Local $Query = "https://api.telegram.org/" & $token & '/sendDocument'
   Local $hOpen = _WinHttpOpen()
   Local $Form = '<form action="' & $Query & '" method="post" enctype="multipart/form-data">' & _
				  ' <input type="text" name="chat_id"/>'  & _
...
Func _SendVoice($ChatID, $Path, $Caption = "")
   Local $Query = "https://api.telegram.org/" & $token & '/sendVoice'
   Local $hOpen = _WinHttpOpen()
   Local $Form = '<form action="' & $Query & '" method="post" enctype="multipart/form-data">' & _
				  ' <input type="text" name="chat_id"/>' & _
...
Func _SendSticker($ChatID,$Path)
   Local $Query = "https://api.telegram.org/" & $token & '/sendSticker'
   Local $hOpen = _WinHttpOpen()
   Local $Form = '<form action="' & $Query & '" method="post" enctype="multipart/form-data">' & _
				  ' <input type="text" name="chat_id"/>' & _
...
Func _SendChatAction($ChatID, $Action)
   Local $Query = "https://api.telegram.org/" & $token & "/sendChatAction?chat_id=" & $ChatID & "&action=" & $Action
   Local $Response = HttpPost($Query)
   ;Return $Response
   Return True
...
Func _SendLocation($ChatID, $Latitude, $Longitude)
   Local $Query = "https://api.telegram.org/" & $token & "/sendLocation?chat_id=" & $ChatID & "&latitude=" & $Latitude & "&longitude=" & $Longitude
   Local $Response = HttpPost($Query)
   ;Return $Response
   Return True
...
Func _SendContact($ChatID,$Phone,$Name)
   Local $Query = "https://api.telegram.org/" & $token & "/sendContact?chat_id=" & $ChatID & "&phone_number=" & $Phone & "&first_name=" & $Name
   Local $Response = HttpPost($Query)
   ;Return $Response
   Return True
...
Func _GetUserProfilePhotos($ChatID,$Offset = "")
   $Query = "https://api.telegram.org/" & $token & "/getUserProfilePhotos?user_id=" & $ChatID
...
Func _GetChat($ChatID)
   Local $Query = "https://api.telegram.org/" & $token & "/getChat?chat_id=" & $ChatID
...
Func _GetFilePath($FileID)
   Local $Query = "https://api.telegram.org/" & $token & "/getFile?file_id=" & $FileID
   Local $Response = HttpPost($Query)
   Local $firstSplit = StringSplit($Response,':')
...
Func _DownloadFile($FilePath)
   ...
   Local $Query = "https://api.telegram.org/file/bot" & $BOT_ID & ":" & $BotToken & "/" & $FilePath
   InetGet($Query,$fileName)
   Return True
EndFunc

#cs ===============================================================================
   Function Name..:		_JSONDecodeTLGM()
