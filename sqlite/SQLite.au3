...
;~ 				$sDll_Filename = "System.Data.SQLite.dll"
			Else
...
Func _SQLite_ErrMsg($hDB = -1)
	...
	If @error Then
		$d=__SQLite_ReportError($hDB, "_SQLite_ErrMsg", Default, "Call Failed")
		ConsoleWrite('!!Debug(' & @ScriptLineNumber & ') : $d = ' & $d & @crlf )
		Return SetError(1, @error, "Library used incorrectly" & @CRLF & $d) ; Dllcall error
	EndIf
	Return $r[0]
...
Func __SQLite_ReportError($hDB, $sFunction, $sQuery = Default, $sError = Default, $vReturnValue = Default, $curErr = @error, $curExt = @extended)
	...
	$sOuterror= "--> Error:    " & $sError & @CRLF
	__SQLite_Print($sOut & @CRLF)
	If Not IsKeyword($vReturnValue) Then Return SetError($curErr, $curExt, $vReturnValue & @CRLF & $sOuterror)
	Return SetError($curErr, $curExt, $vReturnValue & @CRLF & $sOuterror)
